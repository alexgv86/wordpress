...
$footer_column = $supermarket_settings['supermarket_footer_column_section'];
	...
	if ( is_active_sidebar( 'supermarket_footer_options' ) ) :
		...
		<a title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" target="_blank" href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php echo esc_html (get_bloginfo( 'name', 'display' )); ?></a> | 
						...
						<?php  echo '&copy; ' . date_i18n(__('Y','supermarket')) ; ?> <a title="<?php echo esc_attr__( 'WordPress', 'supermarket' );?>" target="_blank" href="<?php echo esc_url( __('https://wordpress.org','supermarket')  );?>"><?php esc_html_e('WordPress','supermarket'); ?></a>
					</div>
	<?php endif; ?>
